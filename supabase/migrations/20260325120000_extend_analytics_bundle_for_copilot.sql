-- Extend CRM Copilot bundle: KPI series (CSV/API), campaigns list, segments, email health,
-- scheduled broadcasts — aligned with Analytics + Dashboard + Campaigns tabs.
...
AS $$
  SELECT jsonb_build_object(
    ...
    'source', 'analytics_bundle_for_copilot_v2',
    ...
    -- KPI time series (braze_kpi_series — REST/CSV); powers Analytics user-growth style metrics
    ...
    -- Campaigns tab / API-synced one-off campaigns
...
COMMENT ON FUNCTION public.analytics_bundle_for_copilot(uuid) IS
  'Analytics JSON for CRM Copilot: campaign analytics CSV, usage/segments, KPI series, braze_campaigns, segments dir, email health, broadcasts. Service role only.';

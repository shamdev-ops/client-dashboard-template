-- Align CRM Copilot canvas metrics with Lifecycle tab:
-- - braze_canvases sample was ORDER BY updated_at DESC LIMIT 50 → often all zeros while
--   workspace sum(entries_last_60d) is huge (high-traffic canvases not in that slice).
-- - Add braze_canvases_totals (full-table sums) + order sample by entries_last_60d DESC.
...
AS $$
  SELECT jsonb_build_object(
    ...
    'source', 'analytics_bundle_for_copilot_v3',
...
COMMENT ON FUNCTION public.analytics_bundle_for_copilot(uuid) IS
  'Analytics JSON for CRM Copilot v3: adds braze_canvases_totals; canvas sample ordered by entries. Service role only.';

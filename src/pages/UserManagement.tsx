...
import { Check, X, Users, ShieldCheck, Clock } from 'lucide-react';
...
export default function UserManagement() {
  ...
  async function handleReject(userId: string) {
    try {
      ...
      // Set is_approved to false (user won't be able to access)
      const { error } = await supabase
        .from('profiles')
        .update({
          is_approved: false,
    ...
    <AppLayout>
                ...
                <TableBody>
                  {pendingUsers.map((pendingUser) => (
                    <TableRow key={pendingUser.id}>
                      ...
                      <TableCell>
                        {format(new Date(pendingUser.created_at), 'MMM d, yyyy')}
                      </TableCell>
                        ...
                        <div className="flex justify-end gap-2">
                          <Button
                            ...
                            onClick={() => setConfirmDialog({
                              ...
                              action: 'approve',
                              userName: pendingUser.full_name || pendingUser.email || 'this user'
                            })}
                ...
                <TableBody>
                  {approvedUsers.map((approvedUser) => (
                    <TableRow key={approvedUser.id}>
                      <TableCell className="font-medium">
                        {approvedUser.full_name || 'Unknown'}
                      </TableCell>
                      ...
                      <TableCell>
                        {approvedUser.approved_at 
                          ? format(new Date(approvedUser.approved_at), 'MMM d, yyyy')
                          : 'Auto-approved'}
                      </TableCell>
                      <TableCell className="text-right">
                        {approvedUser.id !== user?.id && (
                          <Button
                            ...
                            onClick={() => setConfirmDialog({
                              ...
                              action: 'reject',
                              userName: approvedUser.full_name || approvedUser.email || 'this user'
                            })}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {confirmDialog?.action === 'approve' ? 'Approve User' : 'Revoke Access'}
            </AlertDialogTitle>
            <AlertDialogDescription>
